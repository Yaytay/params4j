...
public class SecretsLoader {
  ...
  public static ObjectNode gather(ObjectMapper mapper, Path root, int fileSizeLimit, int fileCountLimit, int fileDepthLimit, Charset charset) throws IOException {
    SecretsWalker visitor = new SecretsWalker(root, mapper, fileSizeLimit, fileCountLimit, charset);
  ...
  private static class SecretsWalker  extends SimpleFileVisitor<Path> {
    ...
    SecretsWalker(Path root, ObjectMapper objectMapper, int fileSizeLimit, int fileCountLimit, Charset charset) {
      this.root = root;
    ...
    public FileVisitResult visitFile(Path file, BasicFileAttributes attrs) throws IOException {
      ...
      if (attrs.isDirectory()) {
        logger.debug("Ignoring file {} because it is a directory (can be caused by max depth being exceeded)", file);
      } else {
        String filename = file.getFileName().toString();
        if (filename.startsWith(".")) {
          logger.trace("Ignoring file {} because it is hidden", file);
        } else if (attrs.size() > fileSizeLimit) {
          logger.debug("Ignoring file {} because it is larger than the maximum size ({} > {})", file, attrs.size(), fileSizeLimit);
        } else if (++fileCount > fileCountLimit) {
          logger.debug("Ignoring file {} because too many files have been processed ({} > {})", file, fileCount, fileCountLimit);
        } else {
          try (FileInputStream fis = new FileInputStream(file.toFile())) {
            String value = new String(fis.readAllBytes(), charset);
            current.put(filename, value);
          } catch(Throwable ex) {
            logger.debug("Ignoring file {} because it could not be read: ", file, ex);          
          }
        }
      }
      return FileVisitResult.CONTINUE;
